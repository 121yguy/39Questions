...
public interface CommentDao {
    ...
    @Select("SELECT comments.id, content, author_id AS authorId, time, icon, nickName FROM comments JOIN my_zhihu_db.user_info u on u.userId = comments.author_id WHERE comments.id < #{startId} ORDER BY id DESC LIMIT 12")
