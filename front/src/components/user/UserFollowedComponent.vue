...
import {cancelSubscribe, getFollowersList, subscribe} from "@/api/user.js";
...
const load = async () => {
  loading.value = true;
  let response = await getFollowersList(uid, page.value);
...
<template>
  <div class="user-list-wrapper" v-infinite-scroll="load" :infinite-scroll-disabled="disabled">
    <ul class="user-list">
      <el-empty v-if="users.length === 0" description="TA还没有关注的人哦..." style="background-color: white"/>
