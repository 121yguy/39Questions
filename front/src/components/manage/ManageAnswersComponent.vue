...
const manageType = 'answers';

onMounted( async () => {
  if ((await checkStatus(manageType)).data) {
    start.value = true;
    answers.value = (await getList(manageType)).data;
    answer.value = answers.value.shift();
  }
})
...
<template>
  ...
  <div class="toolbar" v-if="start">
    <el-button @click="pass(answer.id)" type="success" :icon="Select"></el-button>
    <el-button @click="fail(answer.id)" type="danger" :icon="CloseBold"></el-button>
  </div>
  <div class="content">
    <div v-if="!start">
